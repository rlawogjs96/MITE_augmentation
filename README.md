...
Full A+B (single process):

```bash
python augment_mite_grammar.py --which both --resume
```

Lib B only (faster smoke):

```bash
python augment_mite_grammar.py --which b
```
...
```bash
for i in $(seq 0 7); do
  python augment_mite_grammar.py --which a --shard-index $i --shard-count 8 \
